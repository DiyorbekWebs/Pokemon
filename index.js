...
window.addEventListener('DOMContentLoaded', (e) => {
    e.preventDefault();
    var mapP = pokemons.forEach((item) => {
        list.innerHTML += ` <li class="main__item">
                        <img src="${item.img}" class="main__elemImg" alt="item__img">
                        <div class="main__itemPart">
                            <div class="main__texts">
                                <div class="main__text1">
                                    <h1 class="main__title2">${item.name}</h1>
                                    <p class="main__parag">${item.weaknesses}</p>
                                </div>
                                <div>
                                <div id='bttn' onclick='ok(${item.id})'>
                                 <i id="icon"  class="fa-regular fa-heart main__icon"></i>

                                </div>
                                </div>
                                </div>
                            <div class="main__texts2">
                            <h1 class="main__text2">${item.height}</h1>
                            <h1 class="main__text2">${item.weight}</h1>
                            </div>
                        </div>
                    </li>
        `
...
var arrrr = []
var searchBTn = document.querySelector('.main__btn')
searchBTn.addEventListener('click', (e) => {

    e.preventDefault()
    var select = document.getElementById('select').value;
    var forEach = pokemons.forEach((item) => {
        if (item.type.includes(select)) {
            arrrr.push(item)
        }
        var categoryMap = arrrr.map((item) => {
...
function ok(props) {
    var index = 0;
    var burgerList = document.querySelector('.main__BurgerList');
    var filter = pokemons.filter((item) => {
        if (props === item.id) {
            baza.push(item)
            ...
            <div class="main__itemPart">
                <div class="main__texts">
                    ...
                    <div id='trash' onclick='deleteEl()'>
                    <i  class="fa-solid fa-trash main__icon"></i>
...
var burger = document.querySelector('.main__burger');

function modal() {
    burger.classList.toggle('main__add')
    burger.classList.toggle('active')
}
function ok1() {
    burger.classList.remove('main__add')
}


function dblclick() {
    var mainSearch = document.querySelector('.main__search');

    var searcharr = []
    for (var i = 0; i < pokemons.length; i++) {
        if (mainSearch.value == pokemons[i].name) {
            searcharr.push(pokemons[i]);
            mainSearch.value = null;
        }
    }
    var searchMap=searcharr.map((item)=>{
        return ` <li class="main__item">
                    <img src="${item.img}" class="main__elemImg" alt="item__img">
                    <div class="main__itemPart">
                        <div class="main__texts">
